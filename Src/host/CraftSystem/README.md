...
## Выбираемые рецепты станций

`options.station` (по умолчанию пустая строка) ограничивает рецепт станцией,
чей `getCraftStation` возвращает тот же идентификатор. Для `locks` используется
`LockWorkbench`: ингредиенты со столешницы, один работающий, повторная проверка
условий перед расходом. Обычные рецепты не показываются в меню этой станции.
`options.skill_bonus` (по умолчанию 0) меняет бросок, отдельно от порога
`required.skills`. Станция проверяет порог и при выполнении. Числа рецептов замков
определены в `Crafts/Locks.yml`.
