...
class(DoorLock) extends(Item)
	...
	func(onClick)
	{
		...
		if getSelf(isFastened) exitWith {};
	...
	func(onInteractWith)
	{
		...
		if isTypeOf(_with,Screwdriver) exitWith {
			if !getSelf(isFastened) then {callSelfParams(startToolWork,_usr arg _with)};
		};
		if (!getSelf(isFastened) || {callSelf(lockGrade) == 0}) exitWith {};
		if (isTypeOf(_with,Key) || isTypeOf(_with,KeyChain)) exitWith {
			...
			callFuncParams(_door,setDoorLock,!getVar(_door,isLocked));
		};
		if (callSelf(lockGrade) == 3) exitWith {};
		if isTypeOf(_with,Lockpick) exitWith {callSelfParams(startLockpick,_usr arg _with)};
		if (isTypeOf(_with,Crowbar) && {callSelf(lockGrade) == 1}) then {
			private _roll = callFuncParams(_usr,checkSkill,"ST" arg getVar(_door,stBreakBonus));
			private _result = getRollType(_roll);
			if (_result in [DICE_SUCCESS,DICE_CRITSUCCESS]) then {
				modVar(_with,leftBreaked,-1);
				callSelfParams(playSound,"doors\locked" + str pick [1 arg 2] arg getRandomPitchInRange(0.6,1.3));
				callFuncParams(_usr,meSay,"выламывает замок.");
				callSelf(breakLock);
			};
			if (_result == DICE_CRITFAIL || {getVar(_with,leftBreaked) <= 0}) then {
				delete(_with);
	...
	func(startLockpick)
	{
		...
		private _door = getSelf(door);
		if (!getVar(_door,isLocked) || {getVar(_door,isOpen)}) exitWith {};
	...
	func(startToolWork)
	{
		...
		if (callSelf(lockGrade) == 0 || {isNullReference(getSelf(door))}) exitWith {};
		if (callFuncParams(_usr,getDistanceTo,this) > 2) exitWith {};
		if getVar(getSelf(door),isLocked) exitWith {};
		...
		if !isNullReference(getSelf(toolWorker)) exitWith {};
	...
	func(breakLock)
	{
		...
		["BrokenDoorLock",_pos] call createItemInWorld;
		delete(this);
	};
endclass
...
class(BrokenDoorLock) extends(DoorLock)
	...
	var(desc,"Осталось только погоревать. Починить его уже нельзя.");
